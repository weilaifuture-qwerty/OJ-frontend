  ...
  <div class="student-homework">
    ...
    <div class="homework-container">
      ...
      <div v-else class="empty-state">
        <img src="/public/img/empty-homework.svg" alt="No homework" class="empty-image" />
...
<script>
import { Row, Col, Card, Button, Icon, Input, Badge, Tag, Progress, Spin, Page, Dropdown, DropdownMenu, DropdownItem, Avatar } from 'view-ui-plus'
...
export default {
  name: 'StudentHomeworkListImproved',
  components: {
    Row, Col, Card, Button, Icon, Input, Badge, Tag, Progress, Spin, Page, 
    Dropdown, DropdownMenu, DropdownItem, Avatar
  },
  data() {
    return {
      loading: false,
  ...
  methods: {
    async loadHomework() {
      this.loading = true
      try {
        const res = await api.getStudentHomework({ limit: 100 })
        ...
        if (res && res.data && res.data.data) {
          ...
          this.homeworkList = this.homeworkList.map(h => ({
            ...
            grade: h.status === 'graded' ? Math.floor(Math.random() * 40) + 60 : null
          }))
        }
      } catch (err) {
    ...
    loadMockData() {
      this.homeworkList = [
        ...
        },
        {
          ...
          description: 'Solve classic DP problems to improve your skills',
        ...
        },
        {
          ...
          description: 'Review essential data structures: trees, graphs, and heaps',
        ...
        },
        {
          ...
          description: 'Learn and apply common algorithm design patterns',
          due_date: dayjs().add(14, 'days').toISOString(),
          problem_count: 10,
          completed_count: 10,
          status: 'submitted',
    ...
    calculateStats() {
      ...
      const gradedHomework = this.homeworkList.filter(h => h.grade !== null)
    ...
    isOverdue(homework) {
      return dayjs(homework.due_date).isBefore(dayjs()) && 
             !['submitted', 'graded'].includes(homework.status)
    },
